...
async def clockin(ctx):
    ...
    if not is_allowed_channel(ctx):
        ...
        await ctx.send(f"{ctx.author.mention}, you can only use this command in {allowed_channel.mention}.")
        return
    ...
    for session in sessions:
        if session[1] is None:
            await ctx.send(f"{ctx.author.mention}, you already have an active clock-in. Please clock out first.")
            return
...
async def clockout(ctx):
    ...
    await ctx.send(f"{ctx.author.mention}, you need to clock in first using `/clockin`.")
...
async def worked(ctx, user: discord.Member = None, date: str = None):
    ...
    if report:
        ...
        await ctx.send(f"No records found for {date}.")
...
async def rmv(ctx, user: discord.Member, date: str, index: int):
    ...
    if index < 1 or index > len(sessions):
        await ctx.send(f"{ctx.author.mention}, invalid index. Please provide a valid session index.")
        return
...
async def ongoing(ctx, user: discord.Member = None, action: str = None):
    ...
    if not is_allowed_channel(ctx):
        allowed_channel = ctx.guild.get_channel(ALLOWED_CHANNEL_ID)
        await ctx.send(f"{ctx.author.display_name}, you can only use this command in {allowed_channel.mention}.")
        return
    ...
    if report:
        ...
        await ctx.send("No ongoing work sessions found.")
